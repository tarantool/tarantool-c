...
odbc_parse_dsn(odbc_connect *tcon,SQLCHAR *serv,SQLSMALLINT serv_sz,
	       SQLCHAR *user,SQLSMALLINT user_sz,SQLCHAR *password,
	       SQLSMALLINT password_sz)
{
	...
	SQLGetPrivateProfileString(ret->orig, "HOST", "localhost", ret->host,
				   PARAMSZ, ODBCINI );
	SQLGetPrivateProfileString(ret->orig, "DATABASE", "", ret->database,
				   PARAMSZ, ODBCINI );
	SQLGetPrivateProfileString(ret->orig, "FLAG", "0", ret->flag,
				   PARAMSZ, ODBCINI );
	SQLGetPrivateProfileString(ret->orig, "PORT","0", &port[0],
				   PARAMSZ, ODBCINI );
	...
	if (user) {
		...
		if (!ret->user) 
			goto error;
		SQLGetPrivateProfileString(ret->orig, "USER","", ret->user,
					   PARAMSZ, ODBCINI );
	}
	...
	if (password) {
		...
		if (!ret->password) 
			goto error;
		SQLGetPrivateProfileString(ret->orig, "PASSWORD", "",
					   ret->password, PARAMSZ, ODBCINI );
	}
...
SQLRETURN
odbc_dbconnect (SQLHDBC conn, SQLCHAR *serv, SQLSMALLINT serv_sz, SQLCHAR *user,
		SQLSMALLINT user_sz, SQLCHAR *auth, SQLSMALLINT auth_sz)
{
	...
	tcon->dsn_params=odbc_parse_dsn(tcon,serv,serv_sz,user,user_sz,
					auth,auth_sz);
	if (!tcon->dsn_params) 
		return SQL_ERROR;
