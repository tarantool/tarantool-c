...
tnt_object_container_close (struct tnt_stream *s)
{
	...
	if (sbo->type == TNT_SBO_SIMPLE) {
		...
		if (type == MP_MAP)
			mp_encode_map32(lenp,(uint32_t) size/2);
		else
			mp_encode_array32(lenp, (uint32_t)size);
		return 0;
	} else if (sbo->type == TNT_SBO_PACKED) {
		size_t sz = 0;
		if (type == MP_MAP)
			sz = mp_sizeof_map((uint32_t)(size/2));
		else
			sz = mp_sizeof_array((uint32_t)size);
		if (sz > 1) {
			if (!sb->resize(s, sz - 1))
				return -1;
			lenp = sb->data + offset;
		...
		if (type == MP_MAP) {
			mp_encode_map(sb->data + offset, (uint32_t)(size/2));
		} else {
			mp_encode_array(sb->data + offset, (uint32_t)size);
		}
		sb->size += (sz - 1);
		return 0;
	}
